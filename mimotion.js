 ...
 *
 * 认证方式：读取 .env 里的 PUSH_TOKEN_GITHUB（Personal Access Token），
...
function commitAndPushTokens(dataPath) {
  ...
  try {
    ...
    const token = process.env.PUSH_TOKEN_GITHUB;
    if (!token) {
      console.log('[Git] 未配置 PUSH_TOKEN_GITHUB, 已生成本地 commit 但跳过 push');
      return;
    }
    ...
    fs.writeFileSync(askpassFile, [
      ...
      '  Password*) printf "%s" "$PUSH_TOKEN_GITHUB" ;;',
      'esac',
      '',
    ].join('\n'), { mode: 0o700 });
