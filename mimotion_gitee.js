 ...
 * 该脚本用来做本地部署,和 py 脚本是相同的逻辑和数据存储
 * 本地需要一个 .env 的文件(内容在.env.example中)
 *
 * 功能概述：
...
const TOKEN_CACHE_FILE = 'encrypted_tokens.data';             // token 缓存文件名
...
function getMinMaxByTime(hour, minute, config) {
  const timeRate = Math.min((hour * 60 + minute) / (16 * 60), 1);
 ...
 * 提交并推送 token 缓存到远程仓库
 *
 * 仅在 encrypted_tokens.data 发生变化时执行，与 GitHub Actions 里的
 * cron.yml 行为一致，但 commit 信息以 [local] 开头以区分运行环境。
 * 通过 AUTO_GIT_PUSH=false 可关闭本行为；push 失败不会中断主流程。
 *
 * 认证方式：读取 .env 里的 GIT_PUSH_TOKEN（Personal Access Token），
 * 通过临时 GIT_ASKPASS 脚本喂给 git，避免 token 出现在命令行或日志里。
 * 自动处理 detached HEAD（默认推到 master）。
 *
 * @param {string} dataPath - 加密数据文件路径（用于精确 add 该文件）
 */
function commitAndPushTokens(dataPath) {
  if (process.env.AUTO_GIT_PUSH === 'false') {
    console.log('[Git] AUTO_GIT_PUSH=false, 跳过自动提交');
    return;
  }
  let askpassFile = null;
  try {
    const repoRoot = path.dirname(dataPath);
    const relFile = path.relative(repoRoot, dataPath);
    const status = execSync('git status --porcelain -- ' + JSON.stringify(relFile), { cwd: repoRoot, encoding: 'utf-8' });
    if (!status.trim()) {
      console.log('[Git] 无变更, 跳过提交');
      return;
    }
    execSync('git add -- ' + JSON.stringify(relFile), { cwd: repoRoot, stdio: 'inherit' });

    // 设置 git 用户身份（CI 环境可能未配置）
    execSync('git config user.email "lieryangios@126.com"', { cwd: repoRoot, stdio: 'inherit' });
    execSync('git config user.name "CoderEYLee"', { cwd: repoRoot, stdio: 'inherit' });

    const now = new Date();
    const ts = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
    const message = `[${ts}] [local] persist tokens by packaging machine`;
    execSync(`git commit -m ${JSON.stringify(message)}`, { cwd: repoRoot, stdio: 'inherit' });

    const token = process.env.PUSH_TOKEN_GITEE;
    if (!token) {
      console.log('[Git] 未配置 GIT_PUSH_TOKEN, 已生成本地 commit 但跳过 push');
      return;
    }

    // 解析当前分支；detached HEAD 时降级为 HEAD:master
    let pushRef;
    try {
      pushRef = execSync('git symbolic-ref --short HEAD', { cwd: repoRoot, encoding: 'utf-8' }).trim();
    } catch (e) {
      pushRef = 'HEAD:master';
      console.log('[Git] HEAD 处于 detached 状态, 将推送到 master');
    }

    // 把 origin 转成 HTTPS, 绕开本机没配 SSH key 的问题
    const originUrl = execSync('git config --get remote.origin.url', { cwd: repoRoot, encoding: 'utf-8' }).trim();
    const httpsUrl = originUrl.replace(/^git@github\.com:/, 'https://github.com/');

    // 用临时 askpass 脚本喂 token, 不进入命令行 / ps / 日志
    // git 会分两次调用: 先 Username, 再 Password, 脚本要分别回
    askpassFile = path.join(os.tmpdir(), `mimotion-askpass-${process.pid}-${Date.now()}.sh`);
    fs.writeFileSync(askpassFile, [
      '#!/bin/sh',
      'case "$1" in',
      '  Username*) printf "%s" "lieryang" ;;',
      '  Password*) printf "%s" "$PUSH_TOKEN_GITEE" ;;',
      'esac',
      '',
    ].join('\n'), { mode: 0o700 });
    execSync(`git push ${httpsUrl} ${pushRef}`, {
      cwd: repoRoot,
      stdio: 'inherit',
      env: { ...process.env, GIT_ASKPASS: askpassFile, GIT_TERMINAL_PROMPT: '0' },
    });
    console.log('[Git] 已提交并推送到 origin');
  } catch (e) {
    console.log(`[Git] 自动提交失败: ${e.message}`);
  } finally {
    if (askpassFile) {
      try { fs.unlinkSync(askpassFile); } catch (_) {}
    }
  }
}
...
class MiMotionRunner {
  ...
  async loginAndPostStep(minStep, maxStep) {
    ...
    // 跨天检测 & 步数递增检测
    const tokenInfo = this.userTokens[this.user];
    const lastStep = tokenInfo?.last_step;
    const lastStepTime = tokenInfo?.last_step_time;

    if (lastStep && lastStepTime) {
      const today = new Date().toISOString().slice(0, 10);
      const lastDate = new Date(Number(lastStepTime)).toISOString().slice(0, 10);
      if (today === lastDate) {
        // 同一天：步数必须大于上次
        if (Number(step) <= Number(lastStep)) {
          console.log(`[Runner] 今日已提交 ${lastStep} 步，本次随机 ${step} 步未超过，跳过`);
          this.logStr += `今日已提交 ${lastStep} 步，本次 ${step} 步未超过，跳过\n`;
          return [`今日已提交 ${lastStep} 步，本次 ${step} 步未超过，跳过`, false];
        }
        console.log(`[Runner] 今日已提交 ${lastStep} 步，本次 ${step} 步 > 上次，继续执行`);
      } else {
        console.log(`[Runner] 上次提交: ${lastDate} ${lastStep} 步，已跨天，继续执行`);
      }
    }

    const [ok, msg] = await postFakeBrandData(step, appToken, this.userId);
    if (ok && this.userTokens[this.user]) {
      this.userTokens[this.user].last_step = step;
      this.userTokens[this.user].last_step_time = getTime();
    }
...
async function execute() {
  ...
  const timeBj = getBeijingTime();
  const stepConfig = { MIN_STEP: parseInt(process.env.MIN_STEP) || 20000, MAX_STEP: parseInt(process.env.MAX_STEP) || 20100 };
