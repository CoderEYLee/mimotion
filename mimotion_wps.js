...
function encryptData(plain, key, iv = null) {
  ...
  if (iv === null) {
    ...
    const result = Buffer.concat([randomIv, ciphertext]);
    console.log(`[AES] 加密完成: 密文长度=${result.length} (IV=16 + 密文=${ciphertext.length})`);
    return result;
  } else {
    // 固定 IV，仅返回密文
...
function decryptData(data, key, iv = null) {
  ...
  const result = pkcs7Unpad(decrypted);
...
/**
 * 从本地文件加载已保存的 token 数据（明文 JSON 格式）
 *
 * @param {string} dataPath - 数据文件路径
 * @returns {object} token 数据字典
 */
function prepareUserTokens(dataPath) {
  console.log(`[Token] 【本地存储-读取】加载缓存的 token 数据, 路径=${dataPath}`);
  ...
  console.log(`[Token] 【本地存储-读取】缓存文件不存在, 返回空对象`);
  return {};
}

/**
 * 将 token 数据明文保存到本地 JSON 文件
 *
 * @param {string} dataPath - 数据文件路径
 * @param {object} userTokens - token 数据字典
 */
function persistUserTokens(dataPath, userTokens) {
  const userCount = Object.keys(userTokens).length;
  console.log(`[Token] 【本地存储-写入】持久化 token 数据, 路径=${dataPath}, 账号数=${userCount}`);
  const jsonStr = JSON.stringify(userTokens, null, 2);
  fs.writeFileSync(dataPath, jsonStr, 'utf-8');
  console.log(`[Token] 【本地存储-写入】token 数据已明文保存`);
}
...
async function execute() {
  ...
  // ========== 读取环境变量 ==========
  // 【本地存储】token 缓存文件路径，保存在项目根目录下
  // 文件名: encrypted_tokens.data，格式: 明文 JSON
  const dataPath = path.join(__dirname, 'encrypted_tokens.data');

  // 【本地存储-读取】从文件加载已缓存的 token（明文 JSON 格式）
  const userTokens = prepareUserTokens(dataPath);
  ...
  // 【本地存储-写入】将本次执行获取/更新的 token 明文保存到本地文件
  console.log(`[Main] 【本地存储-写入】持久化 token 缓存到 ${dataPath}`);
  persistUserTokens(dataPath, userTokens);
